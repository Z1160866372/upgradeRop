        ...
        if(pro_config.size()>0){
            ...
            for (ActivityConfiguration config : pro_config) {
                ...
                if(userHistory!=null){
                    ...
                    config.setNoProContent(userHistory.getIp());
                    config.setProContent(userHistory.getIpScanner());
    ...
    public JSONObject submit(String secToken, String actId, int unlocked, String channelId,String ditch) throws Exception {
        ...
        if(commonService.verityTime(actId).equals("underway")) {
            ...
            if (userHistory == null) {
                ...
                if (unlocked == 1||unlocked == 2||unlocked == 4) {//5GB+1GB流量
                    List<ActivityRoster> rosterList = commonMapper.selectRoster(mobile, actId, keyword, unlocked);
