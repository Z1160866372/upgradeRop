...
public interface FoodieMapper {
    ...
    @Insert("insert into wt_newcomerGift_history(userId,unlocked,typeId,rewardName,value,channelId,createDate,createTime,actId,activityId)values(#{userId},#{unlocked},#{typeId},#{rewardName},#{value},#{channelId},#{createDate},#{createTime},#{actId},#{activityId})")
    void insertActivityUserHistory(ActivityUserHistory activityUserHistory);

    @Update("update wt_newcomerGift_history set status=#{status},code=#{code},message=#{message} where id=#{id}")
