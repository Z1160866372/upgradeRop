...
public interface NewCallMapper {
    ...
    @Select("select * from wt_newCall_user_history where userId = #{userId} and unlocked =#{unlocked}")
    ActivityUserHistory selectActivityUserHistoryByUnlocked(@Param("userId")String userId, @Param("unlocked")int unlocked);

    @Insert("insert into wt_newCall_user_history(userId,unlocked,typeId,rewardName,value,channelId,createDate,createTime,actId)values(#{userId},#{unlocked},#{typeId},#{rewardName},#{value},#{channelId},#{createDate},#{createTime},#{actId})")
    void insertActivityUserHistory(ActivityUserHistory activityUserHistory);

    @Update("update wt_newCall_user_history set status=#{status},code=#{code},message=#{message} where id=#{id}")
